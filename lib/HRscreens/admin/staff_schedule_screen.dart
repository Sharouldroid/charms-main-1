...
class _StaffScheduleScreenState extends State<StaffScheduleScreen> {
  ...
  Future<void> _loadSchedules() async {
    ...
    try {
      ...
      debugPrint('Error loading schedules: $error');
  ...
  _ScheduleBadge _resolveBadge(Schedule schedule) {
    ...
    switch (schedule.acceptanceStatus) {
      ...
      default: return _ScheduleBadge('⏳ Pending', Colors.orange);
    }
  }

  Widget _buildStatusBadge(_ScheduleBadge badge) {
    return Container(
          ...
          final result = await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) => ScheduleFormScreen(
                  staffId: _staffId, staffType: _staffType),
            ),
          );
...
class _ScheduleBadge {
  final String label;
  final Color color;
  const _ScheduleBadge(this.label, this.color);
}