...
class _InternTimelineScreenState extends State<InternTimelineScreen> {
  ...
  Future<void> _loadTimelineData() async {
    ...
    try {
      // Step 1: Get registration by userId
      ...
      if (regResponse.statusCode != 200) {
        setState(() {
          _errorMessage = 'Failed to load registration data.';
          _isLoading = false;
        });
        return;
      }
      ...
      if (_scheduleId == null) {
        setState(() {
          _errorMessage = 'No schedule linked to your registration.';
          _isLoading = false;
        });
        return;
      }

      // Step 2: Reuse ScheduleService — fetch all schedules and filter by ID
      // (same approach as ScheduleCalendar, avoids single-item endpoint issues)
      ...
      if (matching.isEmpty) {
        setState(() {
          _errorMessage = 'Your registered schedule could not be found.';
          _isLoading = false;
        });
        return;
      }
  ...
  List<Map<String, dynamic>> get _milestones {
    ...
    return [
      ...
      final milestoneDay = (_totalDays * (m['percent'] as double)).round();
      final milestoneDate = _startDate!.add(Duration(days: milestoneDay));
      final isPast = DateTime.now().isAfter(milestoneDate);
      return {
        ...m,
        'date': milestoneDate,
        'isPast': isPast,
      ...
      child: Column(
        ...
        children: [
          ...
          ...milestones.asMap().entries.map((entry) {
            ...
            final isNext = !isPast &&
                (i == 0 || milestones[i - 1]['isPast'] == true);
