...
class DayTripReport extends StatefulWidget {
  const DayTripReport({
    super.key,
    required this.staff,
    required this.user,
  ...
  @override
  State<DayTripReport> createState() => _DayTripReportState();
}

class _DayTripReportState extends State<DayTripReport> {
  @override
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Day Trip Report'),
          ...
          children: [
            ...
            FutureBuilder(
              future: Provider.of<Reports>(context, listen: false)
                  .fetchMonthlyReport(widget.hostname, 'daytrip'),
            ...
            FutureBuilder(
              future: Provider.of<Reports>(context, listen: false)
                  .fetchYearlyReport(widget.hostname, 'daytrip'),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
                } else {
                  if (snapshot.error != null) {
                    ...
                    return Consumer<Reports>(
                        builder: (ctx, reportdata, child) => ListView.builder(
                            ...
                            itemBuilder: (_, i) => reportdata
                                    .datayear.isNotEmpty
                                ? ListTile(
                                    title:
                                        Text('${reportdata.datayear[i].title}'),
                                    subtitle: Text(
