...
class ExitSurvey extends StatefulWidget {
  const ExitSurvey({
    super.key,
  ...
  @override
  _ExitSurveyState createState() => _ExitSurveyState();
}

class _ExitSurveyState extends State<ExitSurvey> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await Provider.of<Surveys>(context, listen: false).answerExitSurvey(
        widget.hostname,
        combinedAnswers,
        widget.userid,
        widget.eventid,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('${widget.eventname} Exit Survey')),
      body: FutureBuilder(
        future: Provider.of<Surveys>(
          context,
          listen: false,
        ).fetchSurveybyType(widget.hostname, 2, 1),
                  ...
                  builder:
                      (ctx, surveydata, child) =>
                          surveydata.surveylist.isNotEmpty
                              ...
                              : Center(
                                child: const Text('No exit survey available'),
                              ),
                ),
              );
            }
